...
export default function Results({ room, myId, finalPlayers, onPlayAgain, onGoHome, isHost }) {
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      {/* Header */}
      <header style={{ background: 'var(--ink)', padding: 'clamp(0.75rem, 2vw, 1rem) clamp(1rem, 3vw, 2rem)', borderBottom: '3px solid var(--border)' }}>
        <h1 style={{ fontFamily: 'var(--display)', fontSize: 'clamp(1.3rem, 4vw, 2rem)', color: 'var(--paper)', letterSpacing: '0.05em' }}>WIKIRACE</h1>
      </header>

      <div style={{ flex: 1, maxWidth: 700, margin: '0 auto', width: '100%', padding: 'clamp(1.5rem, 3vw, 2.5rem) clamp(0.75rem, 2vw, 1rem)' }}>
        {/* Winner banner */}
        <div style={{ textAlign: 'center', marginBottom: 'clamp(1.5rem, 3vw, 2.5rem)', animation: 'fadeUp 0.4s ease' }}>
          <div style={{ fontFamily: 'var(--mono)', fontSize: 'clamp(10px, 1.5vw, 11px)', color: 'var(--muted)', letterSpacing: '0.2em', marginBottom: '0.5rem' }}>RACE COMPLETE</div>
          {winner ? (
            <>
              <h2 style={{ fontFamily: 'var(--display)', fontSize: 'clamp(1.8rem, 7vw, 5rem)', letterSpacing: '0.05em', color: 'var(--ink)', lineHeight: 1, wordBreak: 'break-word' }}>
                {winner.name.toUpperCase()}
              </h2>
              <p style={{ fontFamily: 'var(--mono)', fontSize: 'clamp(12px, 1.8vw, 14px)', color: 'var(--accent)', marginTop: '0.25rem', letterSpacing: '0.1em' }}>WINS THE RACE</p>
            </>
          ) : (
            <h2 style={{ fontFamily: 'var(--display)', fontSize: 'clamp(1.8rem, 5vw, 3rem)', color: 'var(--muted)', letterSpacing: '0.05em' }}>NO WINNER</h2>
          )}
        </div>
        ...
        <div style={{
          ...
          <div style={{ textAlign: 'center', minWidth: 0 }}>
            <div style={{ fontFamily: 'var(--mono)', fontSize: 'clamp(9px, 1.5vw, 10px)', color: 'var(--muted)', marginBottom: '0.25rem' }}>START</div>
            <div style={{ fontWeight: 600, fontSize: 'clamp(13px, 2vw, 15px)', wordBreak: 'break-word' }}>{room.startArticle}</div>
          </div>
          <div style={{ fontFamily: 'var(--display)', fontSize: 'clamp(1.2rem, 2.5vw, 1.5rem)', color: 'var(--muted)' }}>→</div>
          <div style={{ textAlign: 'center', minWidth: 0 }}>
            <div style={{ fontFamily: 'var(--mono)', fontSize: 'clamp(9px, 1.5vw, 10px)', color: 'var(--muted)', marginBottom: '0.25rem' }}>TARGET</div>
            <div style={{ fontWeight: 600, fontSize: 'clamp(13px, 2vw, 15px)', color: 'var(--accent)', wordBreak: 'break-word' }}>{room.endArticle}</div>
          </div>
        </div>

        {/* Leaderboard */}
        <div style={{ marginBottom: 'clamp(1.5rem, 3vw, 2rem)' }}>
          <div style={{ fontFamily: 'var(--mono)', fontSize: 'clamp(10px, 1.5vw, 11px)', letterSpacing: '0.2em', color: 'var(--muted)', marginBottom: '1rem' }}>FINAL STANDINGS</div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
            {sorted.map((player, i) => {
              const colorIdx = room.players.findIndex(p => p.id === player.id)
              const isMe = player.id === myId
              ...
              return (
                <div key={player.id} style={{
                  ...
                  <div style={{
                    ...
                    color: i === 0 && !player.gaveUp ? 'var(--accent)' : 'var(--paper2)',
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontWeight: 600, fontSize: 'clamp(13px, 2vw, 15px)', wordBreak: 'break-word' }}>
                      {player.name}
                      {isMe && <span style={{ fontFamily: 'var(--mono)', fontSize: 'clamp(10px, 1.5vw, 11px)', color: 'var(--muted)', marginLeft: '0.4rem' }}>(you)</span>}
                    </div>
                    {player.path && player.path.length > 1 && (
                      <div style={{ fontFamily: 'var(--mono)', fontSize: 'clamp(10px, 1.5vw, 11px)', color: 'var(--muted)', marginTop: '0.2rem', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                        {player.path.slice(0, 5).join(' → ')}{player.path.length > 5 ? ` → +${player.path.length - 5} more` : ''}
                      </div>
                    )}
                  </div>
                  ...
                  {/* Stats */}
                  <div style={{ textAlign: 'right', flexShrink: 0 }}>
                    <div style={{ fontFamily: 'var(--mono)', fontSize: 'clamp(12px, 1.8vw, 13px)', fontWeight: 500 }}>{player.clickCount} clicks</div>
                    {finishTime && (
                      <div style={{ fontFamily: 'var(--mono)', fontSize: 'clamp(10px, 1.5vw, 11px)', color: 'var(--muted)' }}>{formatTime(finishTime)}</div>
                    )}
                    {player.gaveUp && (
                      <div style={{ fontFamily: 'var(--mono)', fontSize: 'clamp(10px, 1.5vw, 11px)', color: 'var(--muted)' }}>gave up</div>
                    )}
                  </div>
                </div>
              )
        ...
        {/* Actions */}
        <div style={{ display: 'flex', gap: 'clamp(0.5rem, 1vw, 1rem)', flexWrap: 'wrap' }}>
          {isHost ? (
            <button
              onClick={onPlayAgain}
              style={{
