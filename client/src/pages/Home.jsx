    ...
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      <header style={{ 
        ...
        <div style={{ display: 'flex', alignItems: 'baseline', gap: '0.5rem', minWidth: 0 }}>
          <h1 style={{ fontFamily: 'var(--display)', fontSize: 'clamp(1.5rem, 5vw, 2.5rem)', color: 'var(--paper)', letterSpacing: '0.05em', lineHeight: 1, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>WIKIRACE</h1>
          <span style={{ fontFamily: 'var(--mono)', fontSize: 'clamp(9px, 1.5vw, 11px)', color: '#888', letterSpacing: '0.1em' }}>v1.0</span>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flexShrink: 0 }}>
          <div style={{ width: 8, height: 8, borderRadius: '50%', background: connected ? '#4ade80' : '#ef4444' }} />
          <span style={{ fontFamily: 'var(--mono)', fontSize: 'clamp(10px, 1.5vw, 12px)', color: connected ? '#4ade80' : '#ef4444' }}>{connected ? 'CONNECTED' : 'OFFLINE'}</span>
        </div>
      </header>

      <div style={{ background: 'var(--ink)', padding: 'clamp(1.5rem, 4vw, 3rem) clamp(1rem, 3vw, 2rem)', textAlign: 'center' }}>
        <p style={{ fontFamily: 'var(--mono)', fontSize: 'clamp(9px, 1.5vw, 11px)', letterSpacing: '0.3em', color: 'var(--accent)', marginBottom: '1rem' }}>MULTIPLAYER · REAL-TIME · BROWSER GAME</p>
        <h2 style={{ fontFamily: 'var(--display)', fontSize: 'clamp(1.5rem, 6vw, 4rem)', color: 'var(--paper)', letterSpacing: '0.05em', lineHeight: 1.1, marginBottom: '1rem' }}>RACE THROUGH<br />WIKIPEDIA</h2>
        <p style={{ color: '#aaa', maxWidth: '100%', margin: '0 auto', fontSize: 'clamp(13px, 2vw, 15px)', lineHeight: 1.7 }}>Start at one article. Navigate to the target using only Wikipedia links. First to arrive wins.</p>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(100px, 1fr))', gap: 'clamp(1.5rem, 3vw, 3rem)', marginTop: '2rem', maxWidth: 600, margin: '2rem auto 0' }}>
          {[['UP TO', '8 PLAYERS'], ['NO', 'INSTALL'], ['100%', 'FREE']].map(([label, val]) => (
            <div key={val}>
              <div style={{ fontFamily: 'var(--mono)', fontSize: 'clamp(8px, 1.5vw, 10px)', color: '#666', letterSpacing: '0.15em' }}>{label}</div>
              <div style={{ fontFamily: 'var(--display)', fontSize: 'clamp(1rem, 2.5vw, 1.4rem)', color: 'var(--accent)' }}>{val}</div>
            </div>
          ))}
        </div>
      </div>

      <div style={{ flex: 1, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', padding: 'clamp(1.5rem, 3vw, 2.5rem) clamp(0.75rem, 2vw, 1rem)' }}>
        <div style={{ width: '100%', maxWidth: 460, animation: 'fadeUp 0.4s ease' }}>
          {error && (
            <div style={{ background: '#fff0f0', border: '1.5px solid var(--accent)', borderRadius: 4, padding: '0.75rem 1rem', marginBottom: '1rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontFamily: 'var(--mono)', fontSize: 'clamp(11px, 1.5vw, 13px)', color: 'var(--accent)', gap: '0.5rem' }}>
              <span style={{ flex: 1, overflow: 'hidden', textOverflow: 'ellipsis' }}>{error}</span>
              <button onClick={onClearError} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--accent)', fontSize: '18px', flexShrink: 0 }}>×</button>
            </div>
          )}
          ...
          <div style={{ background: 'white', border: '2px solid var(--border)', borderRadius: 6, overflow: 'hidden', boxShadow: '4px 4px 0 var(--border)' }}>
            <div style={{ display: 'flex', borderBottom: '2px solid var(--border)' }}>
              {[['create', 'CREATE ROOM'], ['join', 'JOIN ROOM']].map(([key, label]) => (
                <button key={key} onClick={() => { setTab(key); onClearError() }} style={{ flex: 1, padding: 'clamp(0.75rem, 2vw, 1rem)', fontFamily: 'var(--display)', fontSize: 'clamp(0.9rem, 2vw, 1.1rem)', letterSpacing: '0.05em', border: 'none', borderBottom: tab === key ? '3px solid var(--accent)' : '3px solid transparent', background: tab === key ? 'var(--paper)' : 'white', color: tab === key ? 'var(--ink)' : 'var(--muted)', cursor: 'pointer', marginBottom: '-2px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{label}</button>
              ))}
            </div>
            ...
            <div style={{ padding: 'clamp(1rem, 2vw, 1.75rem)' }}>
              ...
              {tab === 'create' ? (
                <form onSubmit={handleCreate}>
                  ...
                  <button type="submit" disabled={!playerName.trim() || !connected} style={{ display: 'block', width: '100%', marginTop: '1.5rem', padding: 'clamp(0.6rem, 1.5vw, 0.85rem)', background: 'var(--ink)', color: 'var(--paper)', border: 'none', borderRadius: 4, fontFamily: 'var(--display)', fontSize: 'clamp(0.95rem, 2vw, 1.1rem)', letterSpacing: '0.1em', cursor: 'pointer', opacity: !playerName.trim() || !connected ? 0.5 : 1 }}>→ CREATE ROOM</button>
                </form>
              ) : (
                <form onSubmit={handleJoin}>
          ...
          <div style={{ marginTop: '1.5rem', padding: 'clamp(0.85rem, 2vw, 1.25rem)', border: '1px solid var(--paper2)', borderRadius: 4, background: 'var(--paper2)' }}>
            <div style={{ fontFamily: 'var(--display)', fontSize: 'clamp(0.9rem, 2vw, 1rem)', letterSpacing: '0.1em', marginBottom: '0.75rem', color: 'var(--muted)' }}>HOW TO PLAY</div>
            {['Host creates a room, then picks the start and target articles in the lobby', 'Share the 6-letter code with friends so they can join', 'Everyone navigates Wikipedia by clicking article links', 'First to reach the target article wins!'].map((tip, i) => (
              <div key={i} style={{ display: 'flex', gap: '0.75rem', fontSize: 'clamp(12px, 1.8vw, 14px)', color: 'var(--ink)', marginBottom: '0.4rem' }}>
                <span style={{ fontFamily: 'var(--mono)', color: 'var(--accent)', fontWeight: 500, flexShrink: 0 }}>{i + 1}.</span><span>{tip}</span>
              </div>
            ))}
          </div>
        </div>
