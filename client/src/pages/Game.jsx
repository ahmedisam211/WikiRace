...
export default function Game({ room, myId, onNavigate, onGiveUp }) {
  ...
  const sidebarWidth = isMobile ? '100%' : 240
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', flexDirection: isMobile ? 'column' : 'row' }}>
      ...
      <div style={{ 
        ...
        <div style={{ padding: 'clamp(0.75rem, 1.5vw, 1rem) clamp(0.75rem, 2vw, 1.25rem)', borderBottom: '1px solid #333', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ fontFamily: 'var(--display)', fontSize: 'clamp(1.2rem, 2.5vw, 1.5rem)', letterSpacing: '0.05em' }}>WIKIRACE</div>
          {isMobile && (
            <button onClick={() => setShowSidebar(false)} style={{ background: 'none', border: 'none', color: '#aaa', cursor: 'pointer', fontSize: '20px' }}>✕</button>
          )}
        </div>

        {/* Target */}
        <div style={{ padding: 'clamp(0.75rem, 1.5vw, 1rem) clamp(0.75rem, 2vw, 1.25rem)', borderBottom: '1px solid #333' }}>
          <div style={{ fontFamily: 'var(--mono)', fontSize: 'clamp(9px, 1.5vw, 10px)', color: '#666', letterSpacing: '0.15em', marginBottom: '0.4rem' }}>TARGET</div>
          <div style={{ fontWeight: 600, fontSize: 'clamp(12px, 2vw, 14px)', color: 'var(--accent)', lineHeight: 1.3, wordBreak: 'break-word' }}>{room.endArticle}</div>
        </div>

        {/* My stats */}
        <div style={{ padding: 'clamp(0.75rem, 1.5vw, 1rem) clamp(0.75rem, 2vw, 1.25rem)', borderBottom: '1px solid #333' }}>
          <div style={{ fontFamily: 'var(--mono)', fontSize: 'clamp(9px, 1.5vw, 10px)', color: '#666', letterSpacing: '0.15em', marginBottom: '0.6rem' }}>MY STATS</div>
          <div style={{ display: 'flex', gap: 'clamp(0.75rem, 2vw, 1.25rem)' }}>
            <div>
              <div style={{ fontFamily: 'var(--mono)', fontSize: 'clamp(9px, 1.5vw, 10px)', color: '#666' }}>TIME</div>
              <div style={{ fontFamily: 'var(--mono)', fontSize: 'clamp(1rem, 2vw, 1.2rem)' }}>{formatTime(elapsed)}</div>
            </div>
            <div>
              <div style={{ fontFamily: 'var(--mono)', fontSize: 'clamp(9px, 1.5vw, 10px)', color: '#666' }}>CLICKS</div>
              <div style={{ fontFamily: 'var(--mono)', fontSize: 'clamp(1rem, 2vw, 1.2rem)' }}>{myPlayer?.clickCount || 0}</div>
            </div>
          </div>
          <div style={{ marginTop: '0.75rem' }}>
            <div style={{ fontFamily: 'var(--mono)', fontSize: 'clamp(9px, 1.5vw, 10px)', color: '#666', marginBottom: '0.2rem' }}>CURRENT</div>
            <div style={{ fontSize: 'clamp(11px, 1.5vw, 12px)', color: '#aaa', lineHeight: 1.4, wordBreak: 'break-word' }}>{articleTitle}</div>
          </div>
        </div>

        {/* Players */}
        <div style={{ flex: 1, padding: 'clamp(0.75rem, 1.5vw, 1rem) clamp(0.75rem, 2vw, 1.25rem)' }}>
          <div style={{ fontFamily: 'var(--mono)', fontSize: 'clamp(9px, 1.5vw, 10px)', color: '#666', letterSpacing: '0.15em', marginBottom: '0.75rem' }}>PLAYERS</div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', maxHeight: isMobile ? 'auto' : '40vh', overflowY: 'auto' }}>
            {sortedPlayers.map((player) => {
              const colorIdx = room.players.findIndex(p => p.id === player.id)
              return (
                <div key={player.id} style={{ padding: 'clamp(0.4rem, 0.8vw, 0.6rem) clamp(0.4rem, 0.8vw, 0.6rem)', background: player.id === myId ? '#1a1a1a' : 'transparent', borderRadius: 4, border: player.finishedAt && !player.gaveUp ? '1px solid var(--accent2)' : '1px solid transparent' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.2rem' }}>
                    <div style={{ width: 8, height: 8, borderRadius: '50%', background: PLAYER_COLORS[colorIdx % PLAYER_COLORS.length], flexShrink: 0 }} />
                    <div style={{ fontSize: 'clamp(11px, 1.5vw, 12px)', fontWeight: 500, flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', minWidth: 0 }}>
                      {player.name}{player.id === myId && <span style={{ color: '#555', fontSize: '10px' }}> (you)</span>}
                    </div>
                    {player.place && !player.gaveUp && <span style={{ fontFamily: 'var(--mono)', fontSize: '9px', color: 'var(--accent2)', background: '#0f2b1a', borderRadius: 3, padding: '1px 4px', flexShrink: 0 }}>#{player.place}</span>}
                    {player.gaveUp && <span style={{ fontFamily: 'var(--mono)', fontSize: '9px', color: '#555', flexShrink: 0 }}>out</span>}
                  </div>
                  <div style={{ fontFamily: 'var(--mono)', fontSize: '9px', color: '#555', paddingLeft: '1rem' }}>
                    {player.finishedAt && !player.gaveUp ? '✓ finished' : `${player.clickCount} clicks`}
                  </div>
                </div>
              )
        ...
        {/* Give up / status */}
        <div style={{ padding: 'clamp(0.75rem, 1.5vw, 1rem) clamp(0.75rem, 2vw, 1.25rem)', borderTop: '1px solid #333' }}>
          {!iDone && (
            showGiveUpConfirm ? (
              <div>
                <div style={{ fontFamily: 'var(--mono)', fontSize: 'clamp(10px, 1.5vw, 11px)', color: '#aaa', marginBottom: '0.5rem' }}>Are you sure?</div>
                <div style={{ display: 'flex', gap: '0.5rem' }}>
                  <button onClick={onGiveUp} style={{ flex: 1, padding: '0.4rem', background: 'var(--accent)', color: 'white', border: 'none', borderRadius: 3, cursor: 'pointer', fontFamily: 'var(--mono)', fontSize: 'clamp(10px, 1.5vw, 11px)' }}>YES</button>
                  <button onClick={() => setShowGiveUpConfirm(false)} style={{ flex: 1, padding: '0.4rem', background: '#333', color: '#aaa', border: 'none', borderRadius: 3, cursor: 'pointer', fontFamily: 'var(--mono)', fontSize: 'clamp(10px, 1.5vw, 11px)' }}>NO</button>
                </div>
              </div>
            ) : (
              <button onClick={() => setShowGiveUpConfirm(true)} style={{ width: '100%', padding: '0.5rem', background: 'none', border: '1px solid #333', color: '#555', borderRadius: 3, cursor: 'pointer', fontFamily: 'var(--mono)', fontSize: 'clamp(10px, 1.5vw, 11px)', letterSpacing: '0.1em' }}>GIVE UP</button>
            )
          )}
          {myPlayer?.finishedAt && !myPlayer?.gaveUp && (
            <div style={{ fontFamily: 'var(--mono)', fontSize: 'clamp(10px, 1.5vw, 11px)', color: 'var(--accent2)', textAlign: 'center' }}>✓ YOU FINISHED!</div>
          )}
          {myPlayer?.gaveUp && (
            <div style={{ fontFamily: 'var(--mono)', fontSize: 'clamp(10px, 1.5vw, 11px)', color: '#555', textAlign: 'center' }}>You gave up.</div>
          )}
        </div>
      </div>

      {/* Main content */}
      <div style={{ marginLeft: isMobile ? 0 : 240, marginTop: isMobile && showSidebar ? 0 : 0, flex: 1, padding: 'clamp(1rem, 2vw, 2rem)', maxWidth: 860, position: 'relative', width: isMobile ? '100%' : 'auto' }}>
        {isMobile && (
          <button onClick={() => setShowSidebar(!showSidebar)} style={{ position: 'fixed', bottom: '1rem', right: '1rem', zIndex: 50, width: 50, height: 50, borderRadius: '50%', background: 'var(--ink)', color: 'var(--paper)', border: 'none', cursor: 'pointer', fontFamily: 'var(--display)', fontSize: '1.5rem', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 2px 8px rgba(0,0,0,0.2)' }}>
            {showSidebar ? '✕' : '≡'}
          </button>
        )}

        {/* Breadcrumb */}
        <div style={{ fontFamily: 'var(--mono)', fontSize: 'clamp(10px, 1.5vw, 11px)', color: 'var(--muted)', marginBottom: '1.25rem', lineHeight: 1.8, overflowX: 'auto', whiteSpace: 'nowrap' }}>
          {(myPlayer?.path || []).slice(-5).map((p, i, arr) => (
            <span key={i}>
              <span style={{ color: i === arr.length - 1 ? 'var(--ink)' : 'var(--muted)', fontWeight: i === arr.length - 1 ? 600 : 400 }}>{p}</span>
              {i < arr.length - 1 && <span style={{ margin: '0 0.4rem', color: '#ccc' }}>→</span>}
        ...
        {loading && (
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', padding: '3rem 0', color: 'var(--muted)' }}>
            <div style={{ width: 20, height: 20, border: '2px solid var(--paper2)', borderTopColor: 'var(--ink)', borderRadius: '50%', animation: 'spin 0.7s linear infinite', flexShrink: 0 }} />
            <span style={{ fontFamily: 'var(--mono)', fontSize: 'clamp(12px, 1.8vw, 13px)' }}>Loading {articleTitle}...</span>
          </div>
        )}

        {fetchError && !loading && (
          <div style={{ padding: '2rem 0', color: 'var(--accent)', fontFamily: 'var(--mono)', fontSize: 'clamp(13px, 1.8vw, 14px)' }}>
            Failed to load article: {fetchError}
            <br />
            <button onClick={() => fetchArticle(articleTitle)} style={{ marginTop: '1rem', padding: '0.5rem 1rem', background: 'var(--ink)', color: 'white', border: 'none', borderRadius: 4, cursor: 'pointer', fontFamily: 'var(--mono)', fontSize: 'clamp(11px, 1.5vw, 12px)' }}>Retry</button>
          </div>
        )}
